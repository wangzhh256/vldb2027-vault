...
class OPSDTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        vault_window_size: int = 256,
        vault_min_valid: int = 16,
        vault_init_kappa: float = 0.3,
        vault_init_norm: float = 0.3,
        vault_norm_alpha: float = 0.01,
        vault_c: float = 1.0,
        vault_s_min: float = 0.05,
        vault_valid_w_min: float = 0.05,
        vault_valid_w_max: float = 2.0,
        vault_censored_w_min: float = 0.0,
        vault_censored_w_max: float = 0.1,
        vault_survival_mode: str = "constant",
        vault_bell_sigma: float = 0.5,
        vault_clip_mode: str = "by_validity",
        vault_kappa_update_mode: str = "valid",
        vault_validity_mode: str = "strict",
        vault_weight_mode: str = "vault",
        vault_fixed_censored_weight: float = 0.5,
    ):
        ...
        self.beta = args.beta
        self.rollout_temperature = float(args.temperature)
        self.loss_temperature = float(loss_temperature if loss_temperature is not None else args.temperature)
        self.temperature = self.loss_temperature
        ...
        self.vault_validity_mode = str(vault_validity_mode)
        if self.vault_validity_mode not in {"strict", "boxed"}:
            raise ValueError(f"Unsupported VAULT validity mode: {self.vault_validity_mode}")
        if self.use_vault and self.use_thinking_machines_loss:
            raise ValueError("VAULT currently supports the JSD OPSD loss, not use_tinker_loss.")
        self.vault_weighter = None
        if self.use_vault:
            self.vault_weighter = VaultWeighter(
                window_size=vault_window_size,
                min_valid=vault_min_valid,
                init_kappa=vault_init_kappa,
                init_norm=vault_init_norm,
                norm_alpha=vault_norm_alpha,
                c=vault_c,
                s_min=vault_s_min,
                valid_w_min=vault_valid_w_min,
                valid_w_max=vault_valid_w_max,
                censored_w_min=vault_censored_w_min,
                censored_w_max=vault_censored_w_max,
                survival_mode=vault_survival_mode,
                bell_sigma=vault_bell_sigma,
                clip_mode=vault_clip_mode,
                kappa_update_mode=vault_kappa_update_mode,
                weight_mode=vault_weight_mode,
                fixed_censored_weight=vault_fixed_censored_weight,
            )
        ...
        if self.use_vault:
            print(f"\n{'='*80}")
            print("VAULT MODE ENABLED")
            print("Sequence-level censored learnability weighting is applied to the OPSD loss.")
            print(f"Initial kappa: {self.vault_weighter.kappa}")
            print(f"Initial normalizer: {self.vault_weighter.norm_ema}")
            print(f"Survival mode: {self.vault_weighter.survival_mode}")
            print(f"Clip mode: {self.vault_weighter.clip_mode}")
            print(f"Kappa update mode: {self.vault_weighter.kappa_update_mode}")
            print(f"Validity mode: {self.vault_validity_mode}")
            print(f"Weight mode: {self.vault_weighter.weight_mode}")
            print(f"Fixed censored weight: {self.vault_weighter.fixed_censored_weight}")
            print(f"{'='*80}\n")
    ...
    @staticmethod
    def generalized_jsd_token_values(
        student_logits,
        teacher_logits,
        beta=0.5,
        temperature=1.0,
        ...
        """Return per-token JSD/KL values with shape [batch, seq_len]."""
    ...
    @staticmethod
    def generalized_jsd_k_token_values(
        student_logits,
        teacher_logits,
        beta=0.5,
        temperature=1.0,
        ...
        """Return an unclipped, nonnegative per-token divergence signal for VAULT k."""
        values = OPSDTrainer.generalized_jsd_token_values(
    ...
    def _distributed_is_active(self):
        return (
    ...
    def _vault_update_kappa(self, seq_k, valid):
        local_k = seq_k.detach().float().cpu().tolist()
        local_valid = valid.detach().bool().cpu().tolist()
        if self._distributed_is_active():
            gathered = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(gathered, list(zip(local_k, local_valid)))
            pairs = [item for rank_values in gathered for item in rank_values]
        else:
            pairs = list(zip(local_k, local_valid))
        if self.vault_weighter.kappa_update_mode == "all":
            update_values = [k for k, _ in pairs]
        else:
            update_values = [k for k, is_valid in pairs if is_valid]
        self.vault_weighter.update_kappa_from_values(update_values)

    def _vault_global_valid_raw_mean(self, raw_weights, valid):
        valid = valid.detach().bool()
        local_sum = raw_weights[valid].detach().float().sum()
        local_count = torch.tensor(float(valid.sum().item()), device=raw_weights.device, dtype=torch.float32)
        packed = torch.stack([local_sum.to(dtype=torch.float32), local_count])
        if self._distributed_is_active():
            dist.all_reduce(packed, op=dist.ReduceOp.SUM)
        if packed[1].item() <= 0:
            return None
        return float((packed[0] / packed[1].clamp_min(1.0)).item())

    def _record_vault_metrics(self, seq_k, seq_loss, raw_weights, weights, valid, inputs):
        valid = valid.detach().bool()
        censored = ~valid
        ...
        def append_metric(name, value):
            if value is None:
                return
            self._metrics["train"][name].append(float(value))

        def mean_or_none(tensor, mask=None):
            if mask is not None:
                tensor = tensor[mask]
            if tensor.numel() == 0:
                return None
        ...
        append_metric("vault_kappa", self.vault_weighter.kappa)
        append_metric("vault_norm_ema", self.vault_weighter.norm_ema)
        append_metric("vault_k_mean", mean_or_none(seq_k))
        append_metric("vault_k_median", float(seq_k.detach().float().median().item()))
        append_metric("vault_x_mean", mean_or_none(seq_k.detach().float() / (self.vault_weighter.kappa + self.vault_weighter.eps)))
        append_metric("vault_survival_mean", mean_or_none(self.vault_weighter.survival_prob(seq_k, valid)))
        append_metric("vault_seq_loss_mean", mean_or_none(seq_loss))
        append_metric("vault_raw_w_mean", mean_or_none(raw_weights))
        append_metric("vault_w_mean", mean_or_none(weights))
        append_metric("vault_w_valid_mean", mean_or_none(weights, valid))
        append_metric("vault_w_censored_mean", mean_or_none(weights, censored))
        append_metric("vault_valid_rate", float(valid.float().mean().item()))
        append_metric("vault_censored_rate", float(censored.float().mean().item()))
        append_metric("vault_effective_sample_size", float((weights.sum() ** 2 / weights.square().sum().clamp_min(1e-8)).item()))

        for key, metric_name in [
            ("vault_is_truncated", "vault_truncation_rate"),
            ("vault_has_boxed", "vault_boxed_rate"),
            ("vault_completion_lengths", "generation_length_mean"),
            ("vault_hit_max_length", "generation_hit_max_length_rate"),
        ]:
            value = inputs.get(key)
            if value is not None:
                append_metric(metric_name, mean_or_none(value.detach().float()))

    def _vault_generation_validity(self, generation_ids, completion_texts, completion_mask):
        ...
        if eos_ids is None:
            ...
            eos_id_list = [int(item) for item in eos_ids]
        else:
            eos_id_list = [int(eos_ids)]

        has_eos = torch.zeros(generation_ids.shape[0], device=device, dtype=torch.bool)
        for eos_id in eos_id_list:
            has_eos |= (generation_ids == eos_id).any(dim=1)
        hit_max_length = completion_lengths >= max_completion_length
        is_truncated = hit_max_length & ~has_eos
        has_boxed = torch.tensor(
            [("\\boxed" in text) or ("boxed{" in text) for text in completion_texts],
            device=device,
            dtype=torch.bool,
        )
        if self.vault_validity_mode == "boxed":
            valid = has_boxed
        else:
            valid = has_boxed & ~is_truncated
        return {
            "vault_valid": valid,
            "vault_is_truncated": is_truncated,
            "vault_has_boxed": has_boxed,
            "vault_completion_lengths": completion_lengths.detach().float(),
            "vault_hit_max_length": hit_max_length,
        }
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if return_outputs:
            # Create a loss-only output object to return.
            class LossOnlyOutput:
                def __init__(self):
                    self.loss = None

            loss_only_output = LossOnlyOutput()
        ...
        if self.use_thinking_machines_loss:
            ...
            if self.use_vault:
                token_values = self.generalized_jsd_token_values(
                ...
                seq_loss = masked_sequence_mean(token_values, token_mask)
                k_token_values = self.generalized_jsd_k_token_values(
                    student_logits=student_logits_for_loss,
                    teacher_logits=teacher_logits_for_loss,
                    beta=self.beta,
                    temperature=self.temperature,
                ...
                seq_k = masked_sequence_mean(k_token_values, token_mask).detach()

                valid = inputs.get("vault_valid")
                if valid is None:
                    valid = torch.ones_like(seq_k, dtype=torch.bool)
                valid = valid.to(device=seq_k.device, dtype=torch.bool)

                self._vault_update_kappa(seq_k, valid)
                raw_weights = self.vault_weighter.raw_weights(seq_k, valid).to(device=seq_loss.device)
                valid_raw_mean = self._vault_global_valid_raw_mean(raw_weights, valid)
                self.vault_weighter.update_norm_from_mean(valid_raw_mean)
                weights = self.vault_weighter.normalize_and_clip(raw_weights, valid).to(device=seq_loss.device)

                loss = (weights * seq_loss).sum() / weights.sum().clamp_min(self.vault_weighter.eps)
                self._record_vault_metrics(seq_k, seq_loss.detach(), raw_weights, weights, valid, inputs)
            else:
        ...
        if return_outputs:
            loss_only_output.loss = loss
            return (loss, loss_only_output)
        else:
            return loss
    ...
    def training_step(
        ...
        if self.use_vault:
            ...
            inputs.update(self._vault_generation_validity(generation_ids, completion_texts, completion_mask))
